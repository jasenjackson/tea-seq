...
class junction_map():
    ...
    # same doc string as redundancy_map but different parameters
    ...
    def __init__(self, r1_path, r2_path, ltr, end_size, verbose=True):
        ...
        if not self.end_size > 0:
            ...
            self.build_from_paired()  # locate build_from_paired

        # compute counts
        self.unique_count = len(self.map)
        self.duplicate_count = self.total_count - self.unique_countbuild_from_paired

    def build_from_merged(self, file):  # what is file?
        if self.verbose == True:
            print("Building genome junction map from reads")

        def nested_dict(): return defaultdict(nested_dict)
        # what up with this function here
        # is this some kind of casting thing?
        # look for nested and defaultdict

        self.map = nested_dict()
        #reads = open(file, 'r') fixed memory leak
        # reads was never closed
        with open(file, 'r') as reads:
            while True:
                header = reads.readline()[:-1]  # everything but last char
                seq = reads.readline()[:-1]
                if not seq:  # seq is a boolean?
                    break
                # executed until out of lines I think represented by seq
                self.total_count += 1
                ...
                # look into kmer_search function
    ...
    def build_from_paired(self):
        ...
        def nested_dict(): return defaultdict(nested_dict)
        self.map = nested_dict()  # again with the nested dict thing
        r1, r2 = open(self.fastq_r1, 'r'), open(self.fastq_r2, 'r')
        # r1 and r2 are never closed here
        while True:
            junction = {}
            self.total_count += 1
            ...
            if (self.verbose == True and self.total_count % 1000 == 0):
                print("Processed " + str(self.total_count) + " reads. " +
                      str(len(self.map)) + " junctions identified. ")

            # grab read data
            junction["r1_header"] = r1.readline()[:-1]  # what is being avoided
            junction["r2_header"] = r2.readline()[:-1]  # with [:-1]?
            ...
            r1_pos, r1_dist = kmer_search(
                junction['r1_sequence'], self.ltr, 1, 0)
            r2_pos, r2_dist = kmer_search(
                junction['r2_sequence'], self.ltr, 1, 0)
            if r1_dist <= r2_dist:
                junction = self.add_junction(
                    self.paired_junction(junction, r1_pos, "r1"))
            if r2_dist < r1_dist:
                junction = self.add_junction(
                    self.paired_junction(junction, r2_pos, "r2"))
    ...
    def map_view(self, max_lines=20, feature='depth', view="head", max_char=100):
        ...
        lines_displayed = int()
        if max_lines > self.unique_count:
            lines_displayed = self.unique_count
        else:
            lines_displayed = max_lines
...
def main():
    ...
    lib = junction_map("data/Glycine_GMR30/HL-2_S2_L001_R1_001.fastq",
                       "data/Glycine_GMR30/HL-2_S2_L001_R2_001.fastq", "TGTTAGCCCATA", 30, verbose=True)

    # test functions
    lib.map_view(max_lines=20, feature='depth', view="head", max_char=100)
    print("")
    lib.map_view(max_lines=20, feature='depth', view="tail", max_char=100)
    print("")
...
if __name__ == "__main__":  # remove main here run inside main.py module
    main()

# main function is not defined in this module so what is the above running?
